...
export default function ChecklistSection() {
  const { checklist, toggleChecklist, resetChecklist } = useWorkflow();
  const allDone = Object.values(checklist).every(Boolean);
  const doneCount = Object.values(checklist).filter(Boolean).length;

  return (
    <DashboardCard title="Daily Checklist" icon={<CheckSquare className="h-4 w-4 text-primary" />} delay={700}>
      {/* Progress bar */}
        ...
        {ITEMS.map(item => (
          ...
          >
            ...
            <div className={`flex h-5 w-5 items-center justify-center rounded-md border-2 text-xs transition-all ${
              checklist[item.key] ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/30'
            }`}>
              {checklist[item.key] && '✓'}
            </div>
